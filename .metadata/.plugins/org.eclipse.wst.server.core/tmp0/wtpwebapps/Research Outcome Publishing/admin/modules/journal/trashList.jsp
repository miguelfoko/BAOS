...
<head>
<meta charset="utf-8" />
<title><c:out value="${applicationScope.languageManager.getLanguageValue('journal_trash_list_title', sessionScope.tag)}" /></title>
...
<div>
	<fieldset>
	<legend><c:out value="${applicationScope.languageManager.getLanguageValue('journal_trash_information', sessionScope.tag)}" /> </legend>
		<table border=1>
			<thead>
				<tr>
			...
			<tbody>
				<c:forEach items="${baoJournalOrConf}" var="v">
					<tr>
						...
						<td width="20%"><c:out value="${v['journalOrConfLogo']}"></c:out></td>
